        ...
        <Col md={12}>
          ...
          {userStats.length > 0 ? userStats.map((stats, idx) => (
            <div key={idx}>
              ...
              <p><strong>🎖️ Najwięcej Punktów w Jednej Kolejce: </strong> {stats.maxPointsInOneKolejka}</p>
              <div>
                <Line 
                  data={{
                    labels: stats.kolejki.map((_, index) => `Kolejka ${index + 1}`),
                    datasets: [{
                      label: 'Punkty użytkownika',
                      data: stats.kolejki.map(kolejka => kolejka.points),
